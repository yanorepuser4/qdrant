...
parser.add_argument('--inputs-folder', nargs='+', type=str,
                    default=MRI_FOLDER, help='folder with files')
...
def update_graph(filename_1, filename_2):
    ...
    df_1 = pd.read_json(f'{MRI_FOLDER}/{filename_1}', lines=True)
    df_2 = pd.read_json(f'{MRI_FOLDER}/{filename_2}', lines=True)
    tmp_1 = filename_1.replace(f".{MRI_FORMAT}", '')
    tmp_2 = filename_2.replace(f".{MRI_FORMAT}", '')

    df_load_time = pd.read_csv(f'{MRI_FOLDER}/load_time.csv')
    ...
    df_timeline_1 = parse_log_file(f'{MRI_FOLDER}/{tmp_1}.log')
    df_timeline_2 = parse_log_file(f'{MRI_FOLDER}/{tmp_2}.log')
